...
void Neat::evolve()
{
	...
	if (neatParam.adaptSpeciation == true && generation > 1) // si le user veut qu'on adapte la spéciation 
	{
		// permet de modifier la spéciation tout en restant dans bornes pour quel si reste cohérente
	...
	// calcul le score que chaque espèce.
	for (std::vector<Species>::iterator it = species.begin(); it != species.end(); ++it)
	{
		it->computeScore();
		sortedSpecies.push_back(&*it);

		if (neatParam.elistism == true) // si on souhaite une sélection par élitisme 
		{
			it->sort();
		}
	}

	sortedSpecies.sort(speciesSortAsc()); // on trie les espèces.
	...
	if ((generation % 30) == 0) // toutes les 30 générations, les espèces qui ont une mauvaise perfomance et qui on plus de 20 ans seront détruite.
	{
#ifdef DEBUG
		...
		while (currSpecies != sortedSpecies.end() && (*currSpecies)->age < 20) // on élimine la pire espèce.
		{
			++currSpecies;
		}
	...
	adjustFitness(); // on ajuste le fitness des espèces restantes, sers à répartir le fitness entre les espèces pour éviter qu'une domine les autres
	...
	for (std::vector<Species>::iterator it = species.begin(); it != species.end(); ++it) // permet de savoir combien d'enfant une espèce aura
	{
	...
	if (totalExpected < populationSize) // pour corriger des problèmes de float, si la taille de population est inférieur à celle attendu, on rajout des enfants dans la meileur espèce
	{
	...
	else if (totalExpected > populationSize) // si la taille est plus grande alors on retire des enfants 
	{
		do {
	...
	bestSpecies->setPopChamp(true); // set une espèce comme champion de notre population pour la reproduction
	...
	if (highestLastChanged >= neatParam.dropOffAge + 5) // si la population n'a pas évolué depuis un certain temps, on kill les espèces les plus faibles et on redonnes leurs enfants à l'espèce la plus forte
	{
#ifdef DEBUG
	...
	for (int i = 0; i < populationSize; i++)
	{
		if (genomes[i].getEliminate() == true) // enlève les espèces qui doivent pour mourrir pour ne pas être inclue dans la prochaine reproduction.
		{
			std::list<Species*>::iterator it = sortedSpecies.begin();
	...
	//swap entre les pointeurs de générations pour ne pas faire d'allocation dynamique
	...
	// on supprime les espèces qui n'ont plus génomes ou sinon on incrémente leurs âges 
	...
	// générations des nouveaux réseaux de neuronnes.
	generateNetworks();
}
...
void Neat::reproduce(int workload, std::list<Species*>::iterator it, int newBornIndex, std::list<Species*>& sortedSpecies, Genome* newPop, std::mutex* lock, std::atomic<bool>* ticket)
{
	for(int i = 0; i < workload; ++i, ++it)
	{
		...
		for (int count = 0; count < curSpecies->getExpectedOffspring(); count++, newBornIndex++) 
		{
			...
			// si l'espèce doit avoir plus de 5 enfants et qu'on à pas fait une copie du champ 
			...
			// getSuperChampOffspring c'est quand on fait du delta coding 
			// on garde les 2 meilleurs espèces
			// on fait la mutation de poids ou on rajoute une connexion
			...
			//! on mutation seulement s'il y a qu'un seul génome dans l'espèce ou si le proba fait que c'est true
			else if ((randFloat() < neatParam.pbMutateOnly) || curSpecies->getSpecies()->size() == 1)
			{
				...
				{
					// distribution géométique (regarder sur google) pour sélectionner le premier parent aléatoirement
				...
				// test de probabilité de mutation entre espèce ou non
				...
				{//Mate within Species
					...
					// on essai 10x fois d'avoir un nouveau génome pour la reproduction, si au bout de 10 tentatives il n'a pas trouvé il abandonne
					do {
						if (neatParam.elistism == true)
						{
							index = randGeoDist(std::min(neatParam.rouletteMultiplier / curSpecies->getSpecies()->size(), 0.9999f), curSpecies->getSpecies()->size() - 1);
					...
					// même chose que le give up juste au dessus, mais pour les espèces
					do{
					...
					// sélectionne un génome (voir au-dessus pour les explications du if)
					if (neatParam.elistism == true)
					{
				...
				if (gen2 != gen1)
				{
					...
					// tester la condition de crossover
					...
					// clone le génome 1 si le génome 1 et 2 sont identiques 
					newPop[newBornIndex] = *gen1;
				}

				//Determine whether to mutate the baby's Genome
				//This is done randomly or if the mom and dad are the same organism
				// test voir si on mutate le nouveau génome, ou si le génome 1 et 2
...
void Neat::adjustFitness()
{
	for (std::vector<Species>::iterator itSpecies = species.begin(); itSpecies != species.end(); ++itSpecies)
	{
		int ageDebt = (itSpecies->age - itSpecies->lastImprove + 1) - neatParam.dropOffAge; // défini l'age maximum d'une espèce
		...
		{
			if ((ageDebt >= 1) || itSpecies->obliterate) // si l'espèce doit être tué ou à plus d'un an
			{
				(*itGen)->setSpeciesScore(((*itGen)->getScore()) * 0.01);
			}

			if (itSpecies->age <= 10) // si l'espèce est jeune on augmente son score à l'aide d'un variable qu'on a setter nous même
			{
		...
		{
			(*itGen)->setEliminate(true);//Mark for elimination // élimine les génomes trop mauvais (petit fitness)
		}
	}
}
